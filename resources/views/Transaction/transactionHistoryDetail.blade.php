...
<head>
    ...
    <title>Transaction History Detail Page</title>
</head>
<body style="background-color: #79A3B1">
  ...
  @endphp
    ...
    <div style="margin-left: 25vw; margin-top: 5vh; margin-bottom: 40vh" class="w-50 h-75 ">
      <h3 class="mb-5 text-center fs-1" style="padding-top: 3vh; color: #456268">Your Transaction at {{$transactionsDate->date}}</h3>

      <table class="table" style="background-color: #D0E8F2; border-radius: 25px">
        <thead>
          <tr>
        ...
        <tbody>
          @foreach ($transactionDet as $tran)
          <tr>
            <td><img src="{{ Storage::url($tran->keyboards->image)}}" class="card-img-top" style="border-radius: 15px; width:450px" alt="IMG not found"></td>
            <td>{{$tran->keyboards->name}}</td>
            <td>$ {{$tran->keyboards->price*$tran->quantity}}</td>
            <td>{{$tran->quantity}}</td>
          </tr>
          <h1 style="display: none">{{$tPrice += $tran->quantity * $tran->keyboards->price}}</h1>
          @endforeach
    ...
    <span class="font-weight-bold m-3 d-flex justify-content-center fs-4" style="background-color: #D0E8F2; border-radius: 25px">Total Price: ${{$tPrice}}</span>
